...
-(void) getPointsFromCurrentDrawnLines	{
	...
	if (points_one.count>1)	{
		...
		NSLog(@"Min: %i, %i		Max: %i, %i", minX, minY, maxX, maxY);
		NSLog(@"Size: %i	%i", maxX-minX, maxY-minY);
		box.backgroundColor = [UIColor orangeColor];
		box.frame = CGRectMake(minX, minY, maxX-minX, maxY-minY);
	}
